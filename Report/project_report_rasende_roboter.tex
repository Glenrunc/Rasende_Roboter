...
\subsection{A*}
L'objectif de l'algorithme A* est de rechercher le plus court chemin dans un graphe partant d'un l'état initial pour aller à un état final. En principe on va associer à chaque état une fonction qui donnera une valeur.\\\\ Cette valeur va être ensuite utilisée dans l'algorithme. Cette fonction se décompose en deux parties, un coût qui représente un déplacement du robot et une \textbf{heuristique} qui sert à donner une estimation de la distance qu'il reste à parcourir pour le robot.\\\\
...
C'est-à-dire que tous les robots se rapprocheront de la cible. Comme ça on évite l'implémentation des quêtes secondaires. On poserait évidemment une condition sur le déplacement des robots non principaux pour ne qu'ils prennent pas place sur les cases de poids strictement inférieur à deux.   
